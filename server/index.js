...
// Additional web socket for using plugins - AUTH LOGIC ENHANCED
pluginsWss.on('connection', (ws, request) => { 
    // Fix IP detection for local clients
    let clientIp = request.headers['x-forwarded-for'] || request.connection.remoteAddress || "127.0.0.1";
    const normalizedClientIp = clientIp?.replace(/^::ffff:/, '');

    const userCommandHistory = {};
    
    // ALLOW LOCALHOST connections (internal plugins) but block banned external IPs
    const isLocal = normalizedClientIp === "127.0.0.1" || normalizedClientIp === "::1";
    
    if (!isLocal && serverConfig.webserver.banlist?.includes(normalizedClientIp)) {
      ws.close(1008, 'Banned IP');
      return;
    }
    ...
    ws.on('message', raw => {
        // Anti-spam logic
        const command = helpers.antispamProtection(raw, normalizedClientIp, ws, userCommands, lastWarn, userCommandHistory, '10', 'data_plugins');
        ...
        // ⭐ INJECT AUTHENTICATION DATA HERE
        // This is the crucial part missing in the original file but present in the failed update
        messageData._auth = {
            admin: ws.isAdmin === true,
            tune:  ws.isTune === true,
    ...
    ws.on('close', () => {});
    ws.on('error', error => {
        logError('WebSocket Extra error: ' + error);
    });
});
...
httpServer.on('upgrade', (request, socket, head) => {
  if (request.url === '/text') {
    // Inject session middleware for auth
    sessionMiddleware(request, {}, () => {
        wss.handleUpgrade(request, socket, head, (ws) => {
            // Pass auth flags to WS object
    ...
    sessionMiddleware(request, {}, () => {
      chatWss.handleUpgrade(request, socket, head, (ws) => {
        // Pass auth flags (Chat needs admin status too)
        ws.isAdmin = request.session?.isAdminAuthenticated === true;
    ...
    // ⭐ IMPORTANT: Wrap this in sessionMiddleware to get access to session data
    sessionMiddleware(request, {}, () => {
      pluginsWss.handleUpgrade(request, socket, head, (ws) => {
        // Extract auth from session and attach to WS object
        ws.isAdmin = request.session?.isAdminAuthenticated === true;
        ws.isTune  = request.session?.isTuneAuthenticated === true;
